...
[true, "This person saw a Fuel Depot nearby that should be destroyed.", "SIDE MISSION"] remoteExec ["InA_fnc_formatHint", 0];
...
if (_i == (sideMissionTimer * 5)) exitWith {
	...
	[true, "The Fuel has been moved from its known location and is now untraceable.", "SIDE MISSION"] remoteExec ["InA_fnc_formatHint", 0];
...
while {true;} do {
	...
	if (({alive _x} count (nearestObjects [_objLoc, ["Land_MetalBarrel_F"], 250])) < 12) then {
		...
		[true, "The Fuel has been destroyed.", "SIDE MISSION"] remoteExec ["InA_fnc_formatHint", 0];
